            ...
            for i, (batch_x, batch_x_en, batch_y, batch_x_mark, batch_y_mark, _) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        ...
                        y_true_surrogate = (y_true_norm - beta) / (gamma + 1e-6)  # 逆向剥离乘加混杂污染
                        ...
                        # 联合多任务 Loss 表达式（超参比例稳定设为 1.0 : 0.5 : 0.01）
                        loss = loss_observed + 1.0 * loss_x_surrogate + 0.5 * loss_y_clean + 0.01 * loss_orthogonal
                    ...
                    with torch.no_grad():
                        y_true_norm = self.model.revin1(batch_y.float(), mode='norm')[:, -self.args.pred_len:, -1:]
                    y_true_surrogate = (y_true_norm - beta) / (gamma + 1e-6)
                    loss_y_clean = criterion(y_pred_surrogate, y_true_surrogate)
