...
class DualSpaceSurrogateWrapper(nn.Module):
    def __init__(self, lb_win, h_win, label_win, feature_dim, forecast_model, num_regimes=5):
        """
        ...
        self.regime_prototypes_Y = nn.Parameter(torch.randn(num_regimes, 2 * h_win))
    ...
    def foward_causal(self, x_enc_all, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # 返回核心变量，包括用于外部计算正交损失的 regime_weights  logits
        return x_pred_sel[:, :, :-1], x_out[:, :, :-1], x_out, learnable_mask, y_pred_surrogate, gamma, beta, regime_logits
...
class Exp_Causal_Forecast(Exp_Basic):
    ...
    def train(self, setting):
        ...
        # ===== [前三刀新增] 正则权重（getattr 默认值；未在 args 中定义时取缺省，故既有 run 脚本无需改动）=====
        lambda_balance = getattr(self.args, 'lambda_balance', 0.01)    # Cut1: Switch 风格路由负载均衡（Switch 原文 α=1e-2）
        lambda_entropy = getattr(self.args, 'lambda_entropy', 0.01)    # Cut1: 平均使用率熵正则（推动门控均匀，防坍塌）
        lambda_identity = getattr(self.args, 'lambda_identity', 0.01)  # Cut3: 仿射恒等锚定（值小→可被数据覆盖，弱漂移时退回 ShifTS）
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_x_en, batch_y, batch_x_mark, batch_y_mark, _) in enumerate(train_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        # 调取升级版的统一双空间前向传播接口
                        x_enc_sel, x_enc_pred, outputs, learnable_mask, y_pred_surrogate, gamma, beta, regime_logits = self.model.foward_causal(
                        ...
                        y_true_surrogate = (y_true_norm - beta.detach()) / (gamma.detach() + 1e-6)  # [Cut2] stop-gradient：固定净化目标，gamma/beta 仅经 loss_observed 学习
                        ...
                        # 损失项五[Cut1]：路由负载均衡 + 熵正则（抗路由坍缩；与正交项互补——正交管原型身份、均衡管门控使用率）
                        regime_weights = torch.softmax(regime_logits, dim=-1)               # (B,R) 由 logits 复算软门控
                        num_regimes = regime_weights.shape[-1]
                        P_e = regime_weights.mean(dim=0)                                    # (R,) 批内平均使用率（可微）
                        f_e = F.one_hot(regime_weights.argmax(dim=-1), num_classes=num_regimes).float().mean(dim=0)  # (R,) 硬分配频率（作常数权重）
                        loss_load_balance = num_regimes * torch.sum(f_e.detach() * P_e)     # Switch 风格，均匀使用时取最小
                        loss_route_entropy = torch.sum(P_e * torch.log(P_e + 1e-8))         # = -H(P_e)，最小化即推动使用率趋于均匀

                        # 损失项六[Cut3]：仿射趋同恒等先验（不退化下界——弱漂移时 gamma->1, beta->0，行为回退至 ShifTS）
                        loss_identity = torch.mean((gamma - 1.0) ** 2) + torch.mean(beta ** 2)

                        # 联合多任务 Loss（原 1.0/0.5/0.01 比例保持不变；新增三项默认权重经 getattr 可调）
                        loss = (loss_observed
                                + 1.0 * loss_x_surrogate
                                + 0.5 * loss_y_clean
                                + 0.01 * loss_orthogonal
                                + lambda_balance * loss_load_balance
                                + lambda_entropy * loss_route_entropy
                    ...
                    with torch.no_grad():
                        #y_true_norm = self.model.revin1(batch_y.float(), mode='norm')[:, -self.args.pred_len:, -1:]
                        y_true_norm = self.model.revin1._normalize(batch_y.float())[:, -self.args.pred_len:, -1:]
                    y_true_surrogate = (y_true_norm - beta.detach()) / (gamma.detach() + 1e-6)  # [Cut2] stop-gradient：固定净化目标
                    loss_y_clean = criterion(y_pred_surrogate, y_true_surrogate)
                    ...
                    # 损失项五[Cut1]：路由负载均衡 + 熵正则（抗路由坍缩）
                    regime_weights = torch.softmax(regime_logits, dim=-1)
                    num_regimes = regime_weights.shape[-1]
                    P_e = regime_weights.mean(dim=0)
                    f_e = F.one_hot(regime_weights.argmax(dim=-1), num_classes=num_regimes).float().mean(dim=0)
                    loss_load_balance = num_regimes * torch.sum(f_e.detach() * P_e)
                    loss_route_entropy = torch.sum(P_e * torch.log(P_e + 1e-8))

                    # 损失项六[Cut3]：仿射趋同恒等先验（不退化下界）
                    loss_identity = torch.mean((gamma - 1.0) ** 2) + torch.mean(beta ** 2)
