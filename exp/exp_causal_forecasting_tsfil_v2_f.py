...
class RevIN(nn.Module):
    ...
    该逐通道仿射是每个通道一个标量、跨样本/跨时刻共享，不引入“逐样本逐时刻”的规范自由度。
    """
...
"""
================================ TSFIL v2_f（精简损失版） ================================
在 ShifTS 的 X 空间 SAM 与 RevIN 基础上，融合 FOIL 的 Instance Residual Normalization (IRN)
与 VICReg 跨空间对齐，构成“双空间代理损失融合”。经消融确认：朴素固定权重求和优于此前的
不确定性加权 / 梯度余弦门控 / 各类结构正则，故本版把训练损失固定为：

    t_loss = loss_observed + loss_x_surrogate + 0.1 * loss_align + 0.1 * loss_suf

各分量：
  - loss_observed    : 底座 ERM 观测损失 MSE(Ŷ, Y)，主任务。
  - loss_x_surrogate : ShifTS 的 X 空间代理损失 MSE(X^SUR, X̂^SUR)，由环境条件化 SAM 掩码产生。
  - loss_suf         : FOIL-IRN 残差损失（逐实例统计量 detach，对齐 FOIL 官方源码），净化 Y 空间。
  - loss_align       : VICReg 闭环对齐（variance 项兼作 Y 代理尺度锚，--std_coeff 可调）。

结构沿用：X 空间 SAM、双优化器（底座/路由分组）、利普希茨有界路由（context_encoder + 原型掩码）。
命令行可调：--irn_mode {demean|standardize}（默认 demean）、--std_coeff（默认 1.0）、
           --num_regimes（环境机制数；有真实异质环境的数据如 ILI 可 >1，弱环境如 Exchange 取 1 更优）。
========================================================================================
"""
...
class DualSpaceSurrogateWrapper(nn.Module):
    def __init__(self, lb_win, h_win, label_win, feature_dim, forecast_model, num_regimes=5, proj_dim=128):
        ...
        # 仅保留 X 空间特征代理原型 regime_prototypes_X（Y 空间不含前向路由仿射）。
    ...
    def foward_causal(self, x_enc_all, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # 返回 8 元组：X 空间黄金替代特征真值 X^SUR、去归一化输出的外生分量与整段输出、SAM 掩码、
        #   Y 代理预测 Ŷ^suf、路由 logits，以及 VICReg 两路对齐嵌入 z_align_X / z_align_Y。
...
class Exp_Causal_Forecast(Exp_Basic):
    ...
    def _select_optimizer(self):
        ...
        for name, param in self.model.named_parameters():
            if any(cn in name for cn in causal_component_names):
                ...
                other_parameters.append(param)     # 底座 + Agg + 投影头 等
    ...
    def _vicreg_alignment(self, z_x, z_y, sim_coeff=1.0, std_coeff=1.0, cov_coeff=0.04, eps=1e-4):
        ...
        默认内部系数沿用 VICReg 的 25:25:1 比例（此处等比缩放为 1:1:0.04，整体量级 O(1)）。
        参数:
    ...
    def train(self, setting):
        ...
        # VICReg variance 项系数（Y 代理尺度锚强度），命令行 --std_coeff 可调；projY_std 坍缩(<0.5) 时调大。
        std_coeff = getattr(self.args, 'std_coeff', 1.0)
        # FOIL-IRN 模式：'demean'(默认, 复刻 FOIL exp_informer_final.py) 或 'standardize'(论文 Eq.3 全标准化)。
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_x_en, batch_y, batch_x_mark, batch_y_mark, _) in enumerate(train_loader):
                ...
                def compute_losses():
                    ...
                    # 损失四：VICReg 闭环对齐（非对抗；variance 项=Y 代理尺度锚，std_coeff 命令行可调）
                    loss_align = self._vicreg_alignment(z_align_X, z_align_Y, std_coeff=std_coeff)

                    # ==================== 精简损失合成 ====================
                    # 主观测 + ShifTS X 代理 + 0.1·VICReg 对齐 + 0.1·FOIL-IRN；固定权重求和优于加权/门控（见消融）。
